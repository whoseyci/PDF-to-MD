...
| 17   | **NEW** Deliberation reflector on chart validator | 1.5 | Reflector re-asks chart_extract with tighter params on validator warnings (RecursiveMAS Deliberation pattern; highest expected ROI) | ✅ `pipeline_v2/vision/chart_extract/reflector.py` + `reflective_runner.py`. **After Round 2 fixes: without captions, reflective hits 78.6% (up from 50%)** |
| 18   | **NEW** Auto-OCR in reflective runner | 0.25 | Chart extractors silently fail with `no_axis` when given no OCR; the reflective runner now OCRs on demand | ✅ `auto_ocr=True` default in `run_reflective_extraction`. **Biggest single-fix accuracy lever.** |
